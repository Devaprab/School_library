...
this.getissuedbooks()
  ...
  methods: {
    async issueBook() {
    try {
        const response = await axios.post(`http://192.168.1.7:8080/issues/issue-book?bookId=${this.bookid}&userId=${this.teacherid}`, {
            "bookId": this.bookid,
...
try{
  const response = await axios.get('http://192.168.1.7:8080/issues/allissuedbooks')
  if(response.status===200){
  ...
  console.error(error)
   ...
   async returnBook(index) {
    try {
        ...
        const response = await axios.post(`http://192.168.1.7:8080/return/return-book?bookId=${bookid}&userId=${teacherid}`, {
          "bookId": this.bookid,
          "userId": this.teacherid 
        });
        if (response.status === 200) {
