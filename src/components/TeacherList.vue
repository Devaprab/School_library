  ...
  closeDelete(){
    ...
    async submitEdit(index) {
      ...
      try {
        ...
        const response = await axios.put(`http://192.168.1.7:8080/api/${teacher.id}`, {
    ...
    async deleteTeacher() {
      ...
      try {
        ...
        const response = await axios.delete(`http://192.168.1.7:8080/api/${teacherid}`, {})
        if (response.status === 200) {
          this.dialogDelete = false;
    ...
    async getteachers() {
      try {
        const response = await axios.get('http://192.168.1.7:8080/api/allTeachers')
        if (response.status === 200) {
